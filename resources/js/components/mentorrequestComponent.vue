            ...
            <div class="col-12">
                <div class="card card-body border-sharp shadow-sm sub-card bg-light">
                    <div v-if="!requests.length">
                        <h3 class="text-dark text-left d-inline">You haven't requested for mentorship yet. Browse the mentors page if you need someone to help you! &#128515;</h3>
                    </div>
                        ...
                        <div class="col" v-for="(request, index) in filteredRequests" :key="request.id" v-show="items == 'all' || showItem(index)">
                            <div class="card border-sharp shadow-sm my-3">
                                <div class="card-header d-inline-flex px-0 post-card-header">
                                    <div class="col-auto">
                                        <img :src="'../../../images/avatars/' + getMentor(request).avatar" style="width:50px; height:50px; border-radius:50%;">
                                    </div>
                                    <div class="col-auto my-auto post-card-title">
                                        <h5 class="mb-0 post-title pl-0 text-dark">{{ request.title }}</h5>
                                        <p class="text-muted mb-0">Mentor: {{ getMentor(request).name }}</p>
                                    </div>
                                </div>
                                <div class="card-body post-card-body pb-1">
                                    <p class="badge badge-teal text-white text-left mb-3">{{ getModule(request).code }}</p>
